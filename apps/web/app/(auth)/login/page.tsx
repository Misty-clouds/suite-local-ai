      ...
      <div className="flex w-full flex-col justify-center px-8 sm:px-12 lg:w-1/2 lg:px-24 xl:px-32 relative z-10 bg-[#161616]">
        ...
        <form className="space-y-6" onSubmit={(e) => e.preventDefault()}>
          ...
          <button
            ...
            className={`w-full rounded-full py-4 text-lg font-normal transition-all mt-6 shadow-lg ${
              isFormValid
          ...
          <Link href={"/register"} className="text-[#6E7B82] text-sm font-normal" >Don't have an account?  <span className="text-[#FFFFFF]">Sign up</span></Link>
        </form>

        <div className="absolute bottom-8 left-8 sm:left-12 ">
          <Link href={"/reset-password"} className="flex items-center text-[#AAAAAA] hover:text-white transition-colors gap-2 group">
            <Key className="h-3 w-3 group-hover:scale-110 transition-transform" />
            <span className="text-xs font-light">Reset password</span>
          </Link>
        </div>
      </div>
